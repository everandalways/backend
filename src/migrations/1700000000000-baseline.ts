...
/**
 * Baseline: the complete schema as of 2026-09-25, so an empty database can be
 * built from migrations alone. Before this, the schema existed only inside the
 * production Postgres.
 *
 * Generated from the entity metadata against an empty database (Vendure
 * `generateMigration`), then two edits:
 *  - Timestamped before 1758923070533-stripe so it runs first, and without
 *    customer."customFieldsStripecustomerid", which that migration adds.
 *  - A no-op when the schema already exists. Production (and any restore of
 *    it) already has every table; there TypeORM simply records this migration
 *    as applied. Without the guard, a database that was not marked beforehand
 *    would fail its next boot on the first CREATE TABLE.
 *
 * Not included: facet."customFieldsUsedforvariantoptions". It exists in
 * production but belongs to the unmerged `sync-facet-n-variants` branch
 * (migration 1781137814556, already recorded as applied there), so it is left
 * alone rather than dropped. Vendure logs it as schema drift at boot.
 */
export class Baseline1700000000000 implements MigrationInterface {

   public async up(queryRunner: QueryRunner): Promise<any> {
        if (await queryRunner.hasTable('order')) {
            console.log('[migration] Baseline1700000000000: schema already present — recording as applied, no changes made.');
            return;
        }
